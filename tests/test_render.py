...
class RenderTest(unittest.TestCase):
    ...
    def check_lifecycle_messages(self, text, action, answers):
        ...
        for message in (before_text, after_text):
            ...
            self.assertIn("Cadence repository permissions: Metadata, Contents, Actions = read;", message)
            self.assertIn("Pull requests, Issues, Checks = read & write.", message)
            self.assertIn("Other permissions = no access.", message)
            self.assertIn("OAuth callbacks/client secrets and webhooks/event subscriptions are unused.", message)
            self.assertIn("retain the PEM private key and install the App on each target repo", message)
            self.assertIn("Node 20+ is only needed for the optional", message)
