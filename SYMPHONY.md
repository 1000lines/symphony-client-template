...
Repository: "1000lines/symphony-client-template". Default branch: "main".
...
Start with the [Cadence onboarding skill](.agents/skills/cadence-onboarding/SKILL.md)
for exact credential names/scopes, App installation and grants, environment
admission and readiness evidence. Its native `Symphony Client Setup` probe
validates repository-visible and protected-job credentials without running an AI
review. Verify the installed caller jobs and a real review/check/Linear handoff
...
The owner verifies the created App's grants/IDs and provisions credentials using
the onboarding skill. Rendering is not App registration or credential provisioning.
